...
public class AnimationPanel extends JComponent implements Runnable {
	...
	protected void createNewShape(int x, int y) {
		...
		switch (currentShapeType) { //complete this
			case 0 : {
		        MovingShape newShape = new MovingRectangle(x, y, currentWidth, currentHeight, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath);
		        shapes.add(newShape);
		        break;
		    }
		    case 1 : {
		        MovingShape newShape = new MovingSquare(x, y, currentWidth, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath);
		        shapes.add(newShape);
		        break;
		        
		    }
		    case 2 : {
		        MovingShape newShape = new MovingEllipse(x, y, currentWidth, currentHeight, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath);
		        shapes.add(newShape);
		        break;
		    }
		    case 3 : {
		        MovingShape newShape = new MovingCircle(x, y, currentWidth, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath);
		        shapes.add(newShape);
		        break;
		    }
		    case 4 : {
		        MovingShape newShape = new MovingQuadCircles(x, y, currentWidth, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath);
		        shapes.add(newShape);
		        break;
		    }
		    case 5 : {
		        MovingShape newShape = new MovingPixelArt(x, y, currentWidth, marginWidth, marginHeight, currentBorderColor, currentFillColor, currentPath);
				shapes.add(newShape);
		        break;
		    }
		    default : {
	...
	public String getSortedInfo() {
		//complete this
		String s = "";
	    ArrayList<MovingShape> shapes2 = new ArrayList<MovingShape>(shapes);
	    
	    for(int i=0;i<shapes.size();i++){
	        int j = 1;
	        MovingShape smallerShape = shapes2.get(0);
	        while (j<shapes2.size()){
	            double smallerArea = smallerShape.compareTo(shapes2.get(j));
	            if (smallerArea>0){
	                smallerShape = shapes2.get(j);   
	            }
	            j=j+1;
	        }
	        shapes2.remove(smallerShape);
	        s = s + smallerShape +"\n";
	    }
		return s;
	}
